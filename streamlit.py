...
def page_inicio():
    st.title('''Dashboard sobre el índice S&P500 ''')
    ...
    st.write(' ** El S&P500 es uno de los índices bursátiles más importantes pero no es el único, tenemos también el NASDAQ 100, DOW JONES, entre otros. ** ')
    ...
    st.write('El S&P500 en su trayectoria en los 23 años tuvo tendencia alcista con un crecimiento del 865%, desde el 2000 a la actualidad.')
 #----------------------------------------------------------------------------------------------------------
    # Logica del codigo.
    st.header('Este es un gráfico de barras donde vemos el retorno anual del índice a lo largo de los años.')
    st.write('Cada barra es el retorno de ese año. Las barras por debajo del cero son negativas para ese año, y las barras por encima del cero son positivas para dicho año.')
...
def page_sectores():
    ...
    st.subheader('Sectores que integran el índice S&P500')
    ...
    fig.update_layout(xaxis={'title': 'Sector'},
                    yaxis={'title': 'Frecuencia número de empresas de cada sector'},
                    title='Distribución de sectores del S&P500',
    ...
    st.title('Gráfico de precios de los 5 años de los sectores del S&P500')
    # Mostrar el gráfico
    st.plotly_chart(fig)
...
def page_empresas():
    ...
    st.subheader('Top 10 Empresas del sector Tecnológico y S&P500 en general con mayor rendimiento en 23 años:')
    ...
    st.header('Retornos anuales de las 10 principales empresas tecnológicas')
    # Crear el gráfico de barras
    ...
    fig.update_layout(xaxis={'title': 'Empresa'},
                    yaxis={'title': 'Ganancia (%)'},
                    title='Retorno anual por empresa del sector tecnológico del S&P 500',
                    height=500, width=900)
    st.plotly_chart(fig)
    ...
    # Gráfico de barras de las empresas del S&P 500
    ...
    st.header('Retornos anuales de las 10 principales empresas en general del S&P 500')
    ...
    st.title('Datos importantes de las empresas seleccionadas')
...
def page_kpis():
    st.title('Análisis KPIs Margen de Beneficio')
    ...
    st.subheader('Estadísticas de Margen de Beneficio desde 2010')
...
#---------------------------------KPIS 1 ANSS----------------------------------------------
   ...
   # Agregar una columna para el Margen de Beneficio
    df_a['Margen de Beneficio'] = (df_a['Adj Close'] - df_a['Low']) / df_a['Adj Close']

    st.subheader('Datos de Cotización ANSS')
    # Gráfico de cotización de ANSS
...
#----------------------------------KPIS 2 AAPL ----------------------------------------------------------
    # Analisis KPIS BETA
    st.title('Análisis KPIS BETA')
    st.header('Este KPI nos sirve para medir o ver mejor el nivel de riesgo que puede tener invertir en cada empresa por su relación con la volatilidad del mercado, en este caso las 4 son moderadas con riesgo bajo.')
    ...
    st.write('Tenemos un BETA bajo, es decir, que su volatilidad en relación con el S&P 500 es moderada-baja, lo que es ideal para reducir el riesgo del portafolio recomendado.')
    ...
    # Calcular Beta utilizando el S&P 500 como índice de referencia
    anss_beta = pd.DataFrame({'ANSS': anss['Adj Close'], 'SPY': spy['Adj Close']})
    cov = anss_beta.cov()
    beta = cov.iloc[0, 1] / cov.iloc[1, 1]

    # Crear un gráfico de la acción de ANSS con Plotly
    fig = go.Figure()
...
#-----------------------------------------KPIS 2 CTSH-------------------------------------------------
    ...
    # Calcular Beta utilizando el S&P 500 como índice de referencia
    ctsh_beta = pd.DataFrame({'CTSH': ctsh['Adj Close'], 'SPY': spy['Adj Close']})
    cov = ctsh_beta.cov()
    beta = cov.iloc[0, 1] / cov.iloc[1, 1]

    # Crear un gráfico de la acción de CTSH con Plotly
    fig = go.Figure()
    ...
    # Crear un dashboard en Streamlit
    st.title('Análisis de CTSH')
    st.write('Tenemos un BETA bajo, es decir, que su volatilidad en relación con el S&P500 es moderada baja, lo que es ideal para reducir el riesgo del portafolio recomendado.')
    st.header('Beta (mensual por 5 años): 1.11')
    st.header(f'Beta promedio 23 años: {beta:.2f}')
    st.plotly_chart(fig)
...
#--------------------------------------KPIS 2 MSNT--------------------------------------------------
    ...
    # Calcular Beta utilizando el S&P 500 como índice de referencia
    mnst_beta = pd.DataFrame({'MNST': mnst['Adj Close'], 'SPY': spy['Adj Close']})
    cov = mnst_beta.cov()
    beta = cov.iloc[0, 1] / cov.iloc[1, 1]

   # Crear un gráfico de la acción de MNST con Plotly
    fig = go.Figure()
...
#----------------------------- kPIS 3 AAPL--------------------------------------------------
    st.title('Análisis KPIs - Dividendos pagados')
...
#----------------------------- kPIS 3 ANSS--------------------------------------------------
    # Calcular el dividendo pagado
    anss["Dividend Paid"] = anss["Close"].diff() + anss["Adj Close"].diff()
    anss["Dividend Paid"] = anss["Dividend Paid"].apply(lambda x: x if x > 0 else 0)

    # Crear una tabla de resumen con los KPIs de Dividendos pagados
    dividend_summary = pd.DataFrame({
        "Total Dividends Paid": anss["Dividend Paid"].sum(),
        "Average Dividend Per Share": anss["Dividend Paid"].sum() / len(anss),
        "Dividend Yield": anss["Dividend Paid"].sum() / anss["Close"].mean(),
        "Dividend Payout Ratio": anss["Dividend Paid"].sum() / anss["Adj Close"].sum()
    }, index=[0])
    ...
    # Crear una gráfica de línea para visualizar los dividendos pagados
    dividend_chart = go.Figure()
    dividend_chart.add_trace(go.Scatter(x=anss.index, y=anss["Dividend Paid"], name="Dividendos pagados"))
    dividend_chart.update_layout(title="Dividendos pagados", xaxis_title="Fecha", yaxis_title="Dividendos (USD)")
    st.plotly_chart(dividend_chart)
...
# Calcular el dividendo pagado
    ...
    st.write('Proyección de dividendo y rentabilidad: 1.16 (1.94%)')
    st.write(dividend_summary)
...
# Calcular el dividendo pagado
    ...
    # Crear una tabla de resumen con los KPIs de Dividendos pagados
    dividend_summary = pd.DataFrame({
        "Total de dividendos pagados": aapl["Dividend Paid"].sum(),
        "Promedio de dividendo por acción": aapl["Dividend Paid"].sum() / len(aapl),
        "Rendimiento por dividendo": aapl["Dividend Paid"].sum() / aapl["Close"].mean(),
        "Ratio de pago de dividendos": aapl["Dividend Paid"].sum() / aapl["Adj Close"].sum()
        }, index=[0])
...
def page_conclusion():
    st.header('El equipo de análisis y finanzas concluyó que lo mejor para la empresa es recomendar diversificar su cartera de inversiones en 5 activos, siendo el principal el índice S&P500 con un rendimiento anual promedio del 8% según los últimos años. Además, se analizó que 4 empresas que, aunque también corresponden al S&P500, son de alto rendimiento con bajo nivel de riesgo. Esto crea un equilibrio perfecto para el crecimiento de capital invertido sin tanta especulación de sus rendimientos anuales y permite calcular y proyectar un objetivo de retorno o ganancias.')
...
# Crear un panel de opciones para navegar por las páginas
st.sidebar.title("Selección para Análisis")
selection = st.sidebar.radio("Seleccione página", list(pages.keys()))
